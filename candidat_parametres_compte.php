...
if(isset($_POST['passactuel']) && isset($_POST['nouveaupass']) && isset($_POST['confirmerpass'])){
	if($passactuel == $_SESSION["password"] && $nouveaupass == $confirmerpass){
		$req = $bdd->prepare('UPDATE membres SET password = :pass WHERE nom = :nom AND prenom = :prenom');
		$req->execute(array("pass" => $nouveaupass, "nom" => $_SESSION["nom"], "prenom" => $_SESSION["prenom"]));
...
<h2 style="color:#E57400;">Paramétres de compte</h2>
	<table>
		...
		<td>
			<div class="divLogin">
				<input type="text" value="<?php echo $_SESSION["email"]; ?>" id="stage_sitebundle_candidattype_nom" name="candidat_nom"  class="txtLogin" readonly="readonly" style="background-image: none; background-position: 0% 0%; background-repeat: repeat repeat;padding:0 10px;">
			</div>
		</td>
	</tr>
	</table>
